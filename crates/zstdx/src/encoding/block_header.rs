...
impl BlockHeader {
    ...
    pub fn serialize(self, output: &mut Vec<u8>) {
        ...
        self.serialize_into(&mut prefix);
        output.extend_from_slice(&prefix);
    }

    /// Serialize into a caller-reserved three-byte prefix, for block writers
    /// that reserve the header before the block content's size is known and
    /// patch it in place afterwards.
    pub fn serialize_into(self, prefix: &mut [u8; 3]) {
        ...
        prefix.copy_from_slice(&block_header.to_le_bytes()[0..3]);
    }
}
