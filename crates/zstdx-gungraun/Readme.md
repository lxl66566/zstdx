...
| `benches/small.rs` | `small_fastest` | Repeated one-shot bulk encode of 4-64 KiB payloads (256 calls, state pool warm): the fixed-cost band where allocator churn and per-block entropy builds rival the scan. json/text/random × 4K/8K/16K/64K (json/text), paired zx/zc per cell. |
