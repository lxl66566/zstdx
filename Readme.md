...
2. Malformed input that has been generated by the fuzzer. These don't have to decode (they are garbage) but they must not make the decoder panic


### You want to help fuzz?

The fuzz targets live in `crates/zstdx-fuzz` (`decode`, `decode_dict`, `encode`, `encode_stream`, `interop`, `fse`, `huff0`). From that directory, use `cargo +nightly fuzz run decode` or some other fuzz target to run the fuzzer.

If the fuzzer finds a crash it will be saved to the artifacts dir by the fuzzer. Run `cargo test -p zstdx artifacts` to run the artifacts tests.
