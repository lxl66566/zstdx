...
- **unpledged JobGrid::Growing**: the job starting at absolute offset `o` spans `job_size_for(4o)` — the bulk formula against a geometric estimate of the final size. Jobs grow ~1+2/workers per job past the floor (32MiB@4w: 9 jobs vs bulk's 8 vs 32 at the old fixed floor), so long streams stop paying a per-boundary entropy restart at 1MiB forever while short streams keep the floor's parallelism/latency. The schedule is a pure function of absolute offset: an earlier burst-time growth rule was abandoned because pos-at-burst depends on write chunking, breaking the chunk-independence contract. First version used estimate 2o (14 jobs at 32MiB); 4o measured better (zeros RLE noise +1.26%→+0.10%, text.balanced −0.086%, json.opt −0.30% vs old grid). Residual stream-vs-bulk gap on text.fastest is ±0.1% noise, not boundary-count linear (32→14 jobs closed only 0.06pp).
