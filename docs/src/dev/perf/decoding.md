...
The fused loop is throughput-bound, not latency-bound (IPC 3.45, flat cycle profile, prefetch-negative — attribution in the fused-loop section above): the OOO engine already overlaps decode N+1 with execution N, so the residual is pure loop instruction quality — entry unpacking 6+ uops/sequence, shlx/shrx chains, ~20 live values with residual spilling. The register wall is proven (see negative); every structural rework including the ring pipeline is falsified, so uop-by-uop cuts are the only conversion path — and the easy ones are taken. SIMD copy scheduling, the last structural idea, is now assessed and falsified too (2026-09-15, above): the fused loop is one register-allocation domain, so exec-half copy savings convert 1:1-into-worse decode-half spills. What remains is decode-half uop attrition (entry unpacking, shlx/shrx chains) bought without perturbing the allocation — the falsification also shows even a provably dead branch removal in the exec half nets +0.3-0.4% Ir, so the attrition window is narrow.
