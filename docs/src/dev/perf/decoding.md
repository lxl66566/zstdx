...
The register wall is proven (see negative); every structural rework including the ring pipeline is falsified. The 2026-09-20 round adds the harshest form of the re-roll law: two surgical Ir cuts (real-offset constant rotation, −8.7 Ir/seq; 5-bit nb fields) both lost at the wall — the first to a reproducible +3-5% text-stream placement tax, the second deterministically at Ir (+1.6%) — so per-sequence attrition below ~10 Ir/seq no longer has positive expected value *unless* it cuts the serial chain (the family with a proven conversion record: the fused bit-read traded +4.6% Ir for 2% wall). Remaining read-side levers of that family — folding the reload's window rebuild into the next read, folding `consumed` into `ip` — and the entry-struct layout swap (the one port-mix-changing lever) are ranked in [dec-gap](../bench/dec-gap.md); each needs its own A/B/B/A including text.
