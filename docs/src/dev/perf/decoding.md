...
- **ring-pipeline falsification + throughput-bound attribution (2026-09-14)**: the libzstd `ZSTD_decompressSequencesLong` model (8-deep decode-ahead ring in memory, match-source prefetch, offsets resolved at decode time) was built in two variants (8-slot store-after-exec; 16-slot store-first, so no decoded value stays live across the exec body) and lost on every shape — json.zst3 1380→1105-1231, skewed.zst3 −4..−9%, text.zst3 −1..−4%, dll100.zst19 1350→1162-1233 — with the prefetch alone costing −5% uniformly, exact addresses and 8-deep distance included. The attribution explains why: whole-decode IPC 3.45 (json.zst3, 20 passes) with a flat cycle profile (no instruction above ~3%) — the loop is **throughput-bound, not latency-bound**, so decode N+1 already overlaps exec N through OOO for free, and added bookkeeping (the three FSE states' stack round-trips, the ring store/load, the prefetch-cursor add chain) converts ~1:1 into wall. The loop's L1-miss clusters (wildcopy match source ~8%, its address math ~4.5%, bitstream reload ~4%, FSE entry loads ~3%) all hide behind that overlap — which is also why prefetching them buys nothing. Consequence: the remaining gap to libzstd is instruction quality, and ring/prefetch restructures are falsified (negative.md).

- **SIMD copy scheduling falsified (2026-09-15)**: porting libzstd's `ZSTD_wildcopy` issue structure onto the existing kernels (head-first unconditional copy16 carrying the `ml <= 16` test, 2x-unrolled tail with an exact landing chunk inside the 16-byte overshoot budget, dead `ml > 0` removal; three builds) shrinks the exec half by exactly the predicted 0.5-0.6M Ir (json) yet loses net Ir and wall on every shape — the loop is one register-allocation domain, and the saturated budget returns the copy-side saving as decode-half spills (+0.6-0.8M, the offset_hist base demoted to a per-sequence stack reload). The copy structure is at parity with libzstd's; the residue is decode-half instruction quality under allocation fragility (every source change to either half re-rolls the whole loop's allocation, ±600-800K Ir on json). Data in [negative.md](../negative.md).
