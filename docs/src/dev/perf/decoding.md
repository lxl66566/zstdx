...
The fused loop is throughput-bound, not latency-bound (IPC 3.45, flat cycle profile, prefetch-negative — attribution in the fused-loop section above): the OOO engine already overlaps decode N+1 with execution N, so the residual is pure loop instruction quality — entry unpacking 6+ uops/sequence, shlx/shrx chains, ~20 live values with residual spilling. The register wall is proven (see negative); every structural rework including the ring pipeline is falsified, so uop-by-uop cuts are the only conversion path — and the easy ones are taken. The one conversion path that works *against* rising Ir is cutting segments of the serial spill chain: the fused bit-read (2026-09-19) traded +4.6% Ir for one fewer win store-forwarding round trip per sequence and won 2% wall — but the family's remaining levers are now measured out (2026-09-20): the raw-container read fold and the single-cursor (`consumed`-into-`ip`) fold win the dense shapes ~2% yet reproducibly cost text 3-5% wall, because folding the consumed offset into the count puts the loop cursor on the read's serial chain — the pre-shifted window's count-from-`n` is the shape the long-sequence mix needs. Every read-side structural lever is falsified. What remains is decode-half uop attrition (entry unpacking, shlx/shrx chains) bought without perturbing the allocation — the falsification also shows even a provably dead branch removal in the exec half nets +0.3-0.4% Ir, so the attrition window is narrow.
