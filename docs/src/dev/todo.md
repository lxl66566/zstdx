...
9. **Small/mid payloads (4K-1M)**: json/text mid-size ~2× (per-byte cost of scan+emit+table build, not fixed overhead). Done (2026-09-11): `build_from_weights` counting sort + `package_merge_lengths` scratch rework (u32 Ent, swapped buffers) + per-level package sort removal (the packages are adjacent-pair sums of the sorted previous level, provably pre-sorted; the 10 merge levels are pure O(n) merges now); text-4K ~+11% cumulative, byte-identical output. Remaining: the scan/emit per-byte cost only (back-walk buffer reuse falsified, see negative.md).
