...
6. **Balanced speed** (json 1.54× vs zstd-6 after the gain-aware lazy rework paid ~30% matcher instructions for +7.7% ratio; skewed already overtaken via prefill): the cost is the per-walk second-chance search (2 chain searches per non-improving walk); per-probe/per-position cost of the chain-walk beat-check/unpack and the emit path is the other half (localize with enc_prof + perf). Miss-segment strategy directions (stepping/insertion scheduling) were falsified twice and are closed.
...
13. **Encoder ratio residues** (2026-09-11 ratio sweep + literal-cost-aware store gate; only cells we lose vs libzstd): text.Balanced **−0.57%** (368.1 vs zstd-6's 370.2; the store-gate tension is closed — the literal-cost-aware gate gained text +0.5% and json +0.2% in one move); text.Ultra −0.6% (411.6 vs 413.98, btultra2 price/convergence on huge repeats, marginal); skewed opt/ultra/stream-mt −0.03..−0.09% (16-byte-alphabet fine residue, low priority); everything else at parity or denser (json.Balanced now +15.7% ahead). Anomaly worth understanding: json.opt **bulk-mt is 0.44% denser than bulk-st** (4478237 vs 4498011) — job splitting accidentally helps the opt parser on json (per-job entropy reset / window restart aiding price seeding?); if understood, ST opt may mimic it. Follow-up: the lazy walk's gain comparison still prices literals at the flat `ml*4` — the same code-length feedback could price the walk too.
