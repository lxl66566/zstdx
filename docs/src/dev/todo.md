...
13. **Encoder ratio residues** (2026-09-11 ratio sweep + gain-aware lazy; only cells we lose vs libzstd): text.Balanced **−1.05%** (366.3 vs zstd-6's 370.2; was −2.35% before the offset-priced lazy walk — closed the selection-error half; the rest is the store-gate +7 tension: +4 reaches −0.56% but costs json −5.7%, and the whole swing is 5-byte matches in the 8-32 KiB offset band, i.e. literal-entropy-dependent — a literal-cost-aware gate could break the tension); text.Ultra −0.6% (411.6 vs 413.98, btultra2 price/convergence on huge repeats, marginal); skewed opt/ultra/stream-mt −0.03..−0.09% (16-byte-alphabet fine residue, low priority); everything else at parity or denser (json.Balanced now +13.8% ahead). Anomaly worth understanding: json.opt **bulk-mt is 0.44% denser than bulk-st** (4478237 vs 4498011) — job splitting accidentally helps the opt parser on json (per-job entropy reset / window restart aiding price seeding?); if understood, ST opt may mimic it.
