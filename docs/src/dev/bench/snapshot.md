...
- **Streaming encode MT8**: json.fast/balanced and text.balanced win (x0.35/0.60/0.66); residues are text.fastest x1.90 and json.fastest x1.22 (burst-model serialization, unchanged) plus the best-tier core. The text fast/balanced stream-mt8 cells swing up to 3x between passes (burst-schedule bimodality — 09-12's text.balanced 1546 reproduced on a focused pass).
...
4. **Streaming MT burst-model serialization**: text.fastest x1.90, json.fastest x1.22; the text fast/balanced cells also swing 2-3x between passes (todo 13, persistent-pool redesign).
