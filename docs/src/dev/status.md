...
Known-length sources resize their row (libzstd's `ZSTD_adjustCParams` port: window ≤ srcLog, hash ≤ wlog+1, chain/ring ≤ wlog): bulk paths pass the exact size, streaming passes the pledge, the CLI passes file metadata, unknown sizes keep the row (`Matcher::set_source_hint` / `FrameCompressor::set_size_hint`). Measured on adjusted bands (64KiB-4MiB json/text): ratio parity with libzstd at every probed level (chain rows -5..-15% denser); the 4KiB band keeps a ~+40% fixed-overhead gap on json at this measurement (block/header entropy coding of tiny payloads; ratio since closed to ±1-5% by the small-literal huffman fix, todo 8), while small-call speed wins (4KiB L19 5.96 vs 16.20 ms/call incl. process spawn).
