...
| encode ratio | matched at every tier | 2026-09-16 full-corpus `ratio` sweep vs libzstd at same numeric levels (32 MiB, all modes): geo-mean +8.66% denser; the text.best −0.15..−0.23% cells were closed 2026-09-18 by the cold-head probe step (now +0.12..+0.20%), leaving skewed.opt −0.07% (near-tie) as the one recorded losing residue; text.balanced flipped to +1.65% (cold-start DUBT head); 4KiB json ±1-5%; dll Best/Opt/Ultra all denser than zstd-12/16/19 (ultra +9.8% via optLdm, `--long=26` still ~6% ahead of us) |
